...
class ReverseStringState:
    ...
    def read_token(self, substr_len):
        ...
        try:
            self.token = self.binary_string[self.pos : self.pos + substr_len].decode(
                "utf-8"
            )
        except:
    ...
    def end_with_subtokens(self, subtokens):
        for s in subtokens:
            ...
            logger.debug(
                'output string with subtokens "{:s}"'.format(self.base + self.token + s)
            )
        self.token = ""
...
class SandboxString:
    ...
    def parse_byte_string(self, s, global_vars):
        ...
        while True:
            if rss.state == rss.STATE_UNKNOWN:
                ...
                if (
                    ...
                    == rss.STATE_CONSTANT_READ
                    or rss.state_stack[len(rss.state_stack) - 1]
                    == rss.STATE_RANGE_BYTE_READ
                    or rss.state_stack[len(rss.state_stack) - 1]
                    == rss.STATE_SINGLE_BYTE_READ
                    or rss.state_stack[len(rss.state_stack) - 1] == rss.STATE_PLUS_READ
                ):
                    rss.update_base()
                b = rss.get_next_byte()
                rss.update_state(b)
        ...
        if rss.state == rss.STATE_END_BYTE_READ:
            ...
            logger.warn(
                "last state is not STATE_END_BYTE_READ ({:d})".format(rss.state)
            )
            logger.warn(
                "previous state ({:d})".format(
                    rss.state_stack[len(rss.state_stack) - 1]
                )
            )
        logger.warning(
